...
from transformers.models.bart.modeling_bart import shift_tokens_right, BartModel, BartEncoder, \
    BartEncoderLayer, BartLearnedPositionalEmbedding, BartDecoderLayer
...
class BartCodecEncoder(BartEncoder):
    ...
    def set_codec_weight_init(self, num_of_codec_layers):
        learned_layer_weight_init = (torch.arange(num_of_codec_layers, 0, step=-1).float().
                                     view(num_of_codec_layers, 1, 1, 1))
        self.learning_weight = nn.Parameter(learned_layer_weight_init)
        self.CODEC_RANGE = num_of_codec_layers
    ...
    def forward(
        ...
        if inputs_embeds is None and self.CODEC_RANGE < 2:
            inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale
        else:
            input_ids = input_ids.view(-1, self.CODEC_RANGE, input_ids.shape[-1])
            inputs_embeds = []
            for i in range(self.CODEC_RANGE):
...
class BartCodecForConditionalGeneration(BartForConditionalGeneration):
    ...
    def set_num_codec_layers(self, num_codec_layers):
        self.model.encoder.set_codec_weight_init(num_codec_layers)
