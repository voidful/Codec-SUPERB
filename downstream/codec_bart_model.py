...
class BartCodecEncoder(BartEncoder):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if feature_type_ids is None:
                ...
                mask_0 = feature_type_ids.view(-1) == 0
                mask_1 = feature_type_ids.view(-1) == 1

                input_ids_0 = input_ids.view(-1)[mask_0]
                input_ids_1 = input_ids.view(-1)[mask_1]

                inputs_embeds_0 = self.embed_tokens(input_ids_0) * self.embed_scale
                input_ids_1_reshaped = input_ids_1.view(input_ids.shape[0], Codec_RANGE, -1)

                inputs_embeds_1_list = []
                for i in range(Codec_RANGE):
                    input_scale = self.embed_tokens(input_ids_1_reshaped[:, i, :]) * self.embed_scale
                    inputs_embeds_1_list.append(input_scale)
                weighted_inputs_embeds_1 = torch.mul(torch.stack(inputs_embeds_1_list, dim=0),
                                                     F.softmax(self.learning_weight, dim=0))
                inputs_embeds_1 = torch.sum(weighted_inputs_embeds_1, dim=0)

                total_len = mask_0.sum() + mask_1.sum()
                inputs_embeds = torch.zeros(total_len, self.embed_tokens.embedding_dim).to(input_ids.device)
                inputs_embeds[mask_0] = inputs_embeds_0
                inputs_embeds[mask_1] = inputs_embeds_1

                inputs_embeds = inputs_embeds.view(*input_ids.shape, self.embed_tokens.embedding_dim)
...
class BartCodecDecoder(BartPretrainedModel):
    """
...
class BartCodecModel(BartModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.encoder = BartCodecEncoder(config, self.shared)
        self.decoder = BartCodecDecoder(config, self.shared)
...
class BartCodecForConditionalGeneration(BartForConditionalGeneration):

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartCodecModel(config)
