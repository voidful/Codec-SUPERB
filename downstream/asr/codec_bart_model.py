...
class BartCodecEncoder(BartEncoder):
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.post_init()
    def set_learning_weight(self, codec_book_num: int = 8) -> None:
        self.Codec_RANGE = codec_book_num
        learning_weight_init = torch.arange(self.Codec_RANGE, 0, step=-1).float().view(self.Codec_RANGE, 1, 1, 1)
        self.learning_weight = nn.Parameter(learning_weight_init).to(self.learning_weight.device)
    def forward(
            self,
